...
The Identity of Fractions: We need a concise way to recognize the following idea, so lets call this concept or rule "The Identity of Fractions": `x = x/1`, and it is saying that any number or variable can be rewritten as itself over `1`. This rule highlights the inherent flexibility in the way we represent numbers, and it’s especially useful when working with fractions or algebraic expressions. It emphasizes that the value of the number doesn't change when you rewrite it this way (we are going to be exploiting this in EVERYTHING going forward)
...
Often, when we solve an equation, we focus on manipulating one side to isolate a variable. But the goal is still to find the relationship between the two sides. For example, in the equation `3x+5=11`, we might subtract  5  from both sides to isolate  the  `3x`  term, ultimately solving for  `x`
...
Sometimes, you might have the right side of an equation but need to reverse-engineer or work backward from the solution to deduce the input or conditions. For example, in optimization problems, you may know the result and need to backtrack to find the conditions that produce that result. This is often seen in systems of equations or inverse functions.

Specifically in systems of equations (or working w/ multiple equations simultaneously), we’re looking for relationships that satisfy all equations in a system. The LHS and RHS interact across multiple equations, and the solution is typically the values that satisfy all these relationships simultaneously.

In conclusion, the rules governing each side of an equation do not always carry over into other problems, contexts and circumstances. And therefore, it is the context of what you are doing that is itself very much integral—the primary component that you are basing all your intuition on in order to advance the narrative behind that particular equation or set of equations.
...
When we talk about orientation in terms of the Cartesian plane, we’re discussing how points, lines, and shapes are positioned and rotated in space. The coordinate system Descartes introduced lets us identify the angles between lines by using the concept of slope and, through trigonometry, determine the precise angle between two lines.
...
signum(x) = { -1   if x < 0
               ...
               1   if x > 0
```
**Splines**, or spline functions are inherently complex. There are different kinds of spline functions: Linear, bezier, b-splines, quadratic, cubic... I'll briefly talk about bezier splines (bezier curves) when we get intro quadratic functions, parametric curves, etc. Linear splines are just straight lines that go through each point, while B-splines are smooth curves, that which defines smooth shapes (w/ control points) In simpler terms, b-splines and bezier curves often coincide; And dropping down two control points enables you to grab the edges and move it around as a fixed line (spline) or curvy line (bezier) usually represented w/ the "pen tool" in computer applications. After setting an initial control point, and before a beziers path has been closed, it behaves like an interactive, flexible curve (or what i like to call) the act of taming a wet noodle. That probably gives you a clear picture of what im talking about, as it exists in most common applications.
